...
class TestTableModule:
    ...
    def create_test_button(self, name, bit):
        """ CORREGIDA: Crea botón de prueba que envía comando Modbus Y maneja timer"""
        def on_click(e):
            print(f"[BOTÓN] Presionado: {name} (M{bit})")
            
            #  LLAMAR A LA FUNCIÓN CORRECTA SEGÚN EL BIT
            if bit == 269:  # Botón "Iniciar Prueba"
                self._on_start_test(e)  #  USAR LA FUNCIÓN COMPLETA
            else:
                #  OTROS BOTONES USAN EL SISTEMA ORIGINAL
    ...
    def set_test_type_from_calibration(self, test_type):
        """ MEJORADA: Establece el tipo de prueba Y notifica al módulo de valores instantáneos"""
        print(f"[TEST_TABLE]  Estableciendo tipo de prueba por calibración: {test_type}")
        
        self.current_test_type = test_type
        
        #  NOTIFICAR AL MÓDULO DE VALORES INSTANTÁNEOS SOBRE EL CAMBIO
        if self.instant_values_module and hasattr(self.instant_values_module, 'set_current_test_type'):
            self.instant_values_module.set_current_test_type(test_type)
            print(f"[TEST_TABLE]  Tipo de prueba notificado a instant_values: {test_type}")
        else:
            print(f"[TEST_TABLE]  No se pudo notificar tipo de prueba a instant_values")
        ...
        if self.timer_module:
            ...
            print(f"[TEST_TABLE]  Timer module no disponible")
        ...
        print(f"[TEST_TABLE]  Tiempo configurado: {estimated_time_minutes:.1f} minutos")
        print(f"[TEST_TABLE]  Módulo de valores instantáneos notificado sobre Q3/Q4 lógica")
        ...
        try:
            ...
            if hasattr(self, 'page') and self.page and hasattr(self.page, 'session'):
                try:
                    ...
                    print(f"[TEST_TABLE]  Error accediendo a sesión: {e}")
            ...
            print(f"[TEST_TABLE]  Calculando tiempo para {test_type}...")
    ...
    def calculate_estimated_time_from_flow(self, test_type, default_volume=100):
        ...
        try:
            ...
            if base_flow <= 0:
                print(f"[TEST_TABLE]  Caudal para {test_type} es 0, usando default")
            ...
            print(f"[TEST_TABLE]  Tiempo calculado para {test_type}: {default_volume}L / {qmax:.2f}L/h = {time_minutes:.1f} min")
    ...
    def set_test_type_from_button(self, test_type):
        ...
        self.update_table()
        print(f"[TEST_TABLE]  Tipo de prueba establecido: {test_type}")
        print(f"[TEST_TABLE]  Tiempo estimado configurado: {estimated_time:.1f} minutos")
        ...
        try:
            ...
            if self.instant_values_module:
                try:
                    ...
                    print(f"[TEST_TABLE]  Error obteniendo valor del módulo: {e}")
    ...
    def _show_validation_alert(self, empty_rows):
        ...
        message = f" Las siguientes filas tienen seriales pero no lecturas iniciales:\n\n{rows_text}\n\nPor favor, complete las lecturas iniciales antes de iniciar la prueba."
    ...
    def remove_row(self, idx):
        ...
        if len(self.rows) > 1 and idx < len(self.rows):
            ...
            print(f"[TEST_TABLE]  No se puede eliminar fila {idx} (total: {len(self.rows)})")
    ...
    def _on_start_test(self, e):
        """ CORREGIDA: Inicia la prueba con timer de cuenta regresiva"""
        #  VALIDAR QUE HAYA UN TIPO DE PRUEBA SELECCIONADO
        if not self.current_test_type:
            print("[TEST_TABLE]  No hay tipo de prueba seleccionado")
        ...
        if not has_serial:
            print("[TEST_TABLE]  No hay seriales ingresados")
            self._show_error_alert(
                " Seriales Requeridos",
        ...
        #  CONFIGURAR EL TIMER CON EL TIEMPO CORRECTO
        print(f"[TEST_TABLE]  Configurando timer con {configured_time:.1f} minutos para {self.current_test_type}")
        self.timer_module.set_time(configured_time)
        
        #  ENVIAR COMANDO M269 (INICIAR PRUEBA) PRIMERO
        try:
            print("[TEST_TABLE]  Enviando comando M269 (Iniciar Prueba)")
            self.send_bool_m(269)
        except Exception as ex:
            print(f"[TEST_TABLE]  Error enviando comando M269: {ex}")
        
        #  PEQUEÑA PAUSA PARA ASEGURAR QUE EL COMANDO SE ENVIÓ
        import time
        time.sleep(0.5)
        
        #  AHORA SÍ INICIAR LA CUENTA REGRESIVA
        print(f"[TEST_TABLE]  Iniciando cuenta regresiva de {configured_time:.1f} minutos...")
        try:
            self.timer_module.start_countdown()
            print(f"[TEST_TABLE] ⏰ Cuenta regresiva iniciada exitosamente")
        except Exception as timer_error:
            print(f"[TEST_TABLE]  Error iniciando timer: {timer_error}")
            self._show_error_alert(
                " Error del Timer",
                f"No se pudo iniciar el timer: {str(timer_error)}"
            )
            return
        ...
        if hasattr(self, 'on_test_control'):
            try:
                ...
                print(f"[TEST_TABLE]  Error en callback: {callback_error}")
        ...
        try:
            ...
            print(f"[TEST_TABLE]  Error actualizando botones: {update_error}")
            
        print(f"[TEST_TABLE] ▶️ Prueba iniciada exitosamente: {self.current_test_type}")
        print(f"[TEST_TABLE] ⏰ Timer configurado para: {configured_time:.1f} minutos")
        
        #  DEBUG: VERIFICAR ESTADO DEL TIMER
        if hasattr(self.timer_module, 'is_running'):
            print(f"[TEST_TABLE]  Timer está corriendo: {self.timer_module.is_running}")
            print(f"[TEST_TABLE]  Tiempo restante: {self.timer_module.remaining_time} segundos")
    ...
    def debug_show_configurations(self):
        ...
        try:
            ...
            if hasattr(self, 'test_configurations_direct') and self.test_configurations_direct:
                ...
                print(f"[TEST_TABLE]  No hay configuraciones directas")
            ...
            if hasattr(self, 'page') and self.page and hasattr(self.page, 'session'):
                try:
                    if hasattr(self.page.session, 'get'):
                        ...
                        print(f"[TEST_TABLE]  Sesión no tiene método get")
                except Exception as e:
                    print(f"[TEST_TABLE]  Error accediendo sesión: {e}")
            else:
                print(f"[TEST_TABLE]  No hay acceso a sesión")
    ...
    def _show_validation_alert(self, empty_rows):
        ...
        rows_text = ", ".join([f"#{row}" for row in empty_rows])
        message = f" Las siguientes filas tienen números de serie pero no tienen lecturas iniciales:\n\n🔢 Filas: {rows_text}\n\n📝 Por favor, complete las lecturas iniciales antes de iniciar la prueba."
...
Los datos han sido guardados y la tabla está lista para la siguiente prueba."""
    ...
    def _on_finish_test(self, e):
        ...
        if not self.current_test_type:
            print("[TEST_TABLE]  No hay tipo de prueba para finalizar")
            return
    ...
    def build(self):
        ...
        if self.timer_module:
            ...
            control_buttons.append(
                ft.Container(
                    ft.Text(" Timer: No disponible", color=ft.Colors.GREY),
                    width=140,
                    height=40,
                    alignment=ft.alignment.center,
                    border_radius=8,
    ...
    def _send_end_tests_command(self):
        ...
        try:
            ...
            if self.send_modbus_command:
                ...
                print(f"[TEST_TABLE]  Callback Modbus no disponible")
        ...
        try:
            ...
            if hasattr(self, 'page') and self.page and hasattr(self.page, 'session'):
                try:
                    ...
                    print(f"[TEST_TABLE]  Error accediendo a session: {e}")
            
            #  DATOS POR DEFECTO COMO ÚLTIMA OPCIÓN
            print("[TEST_TABLE]  Usando datos por defecto - no se encontraron datos de sesión")
            return {
